...
def get_suitabilities() -> dict[list[dict]]:
    ...
    with open('DT_PalMonsterParameter.json') as file:
        pal_parameters: dict[dict] = json.load(file)['Rows']

    with open('DT_PalNameText.json') as file:
        pal_names: dict[dict] = json.load(file)['Rows']
    ...
    for pal, data in pal_parameters.items():
        ...
        suitability_levels = {
            ...
            # 'defrost_level' : data["WorkSuitability_Deforest"],
